...
function rgba(hex,a){if(hex==null||hex.length<7)return "rgba(79,142,247,"+a+")";const r=parseInt(hex.slice(1,3),16),g=parseInt(hex.slice(3,5),16),b=parseInt(hex.slice(5,7),16);return "rgba("+r+","+g+","+b+","+a+")";}

export default function AgendaView(){
  const [token,setToken]=useState(()=>localStorage.getItem("ag_tok2"));
  const [email,setEmail]=useState(()=>localStorage.getItem("ag_em2"));
  ...
  useEffect(()=>{
    ...
    if(hash.includes("access_token")==false)return;
    ...
    fetch("https://www.googleapis.com/oauth2/v2/userinfo",{headers:{Authorization:"Bearer "+t}})
      .then(r=>r.json()).then(info=>{
        const em=info.email||"";
        localStorage.setItem("ag_tok2",t);
        localStorage.setItem("ag_em2",em);
        setToken(t);setEmail(em);
        loadEvents(t);
      });
  },[]);

  useEffect(()=>{if(token!=null)loadEvents(token);},[]);
  ...
  const loadEvents=async(t)=>{
    ...
    if(r.ok==false){localStorage.removeItem("ag_tok2");localStorage.removeItem("ag_em2");setToken(null);setEmail(null);setLoading(false);return;}
    const d=await r.json();
    setEvents(d.items||[]);
    setLoading(false);
  };

  const disconnect=()=>{localStorage.removeItem("ag_tok2");localStorage.removeItem("ag_em2");setToken(null);setEmail(null);setEvents([]);};
  ...
  const saveEvent=async()=>{
    ...
    if(f.start!=null)body.start={dateTime:new Date(f.start).toISOString(),timeZone:"Europe/Paris"};
    if(f.end!=null)body.end={dateTime:new Date(f.end).toISOString(),timeZone:"Europe/Paris"};
  ...
  const deleteEvent=async()=>{
    if(window.confirm("Supprimer ?")==false)return;
  ...
  const evTop=e=>{if(e.start?.dateTime==null)return 0;const d=new Date(e.start.dateTime);return Math.max(0,(d.getHours()+d.getMinutes()/60-7)*64);};
  const evHt=e=>{if(e.start?.dateTime==null||e.end?.dateTime==null)return 32;return Math.max(28,(new Date(e.end.dateTime)-new Date(e.start.dateTime))/60000/60*64);};
                ...
                {byDay(day).map(evt=>{
                  ...
                  return(
                    <div key={evt.id} className={styles.event}
                      ...
                      {evt.start?.dateTime!=null&&<div className={styles.eventTime}>{fmt(evt.start.dateTime)}</div>}
                    </div>
                  );
                })}
              </div>
